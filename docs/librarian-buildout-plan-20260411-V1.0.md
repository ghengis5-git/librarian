...
This document is the authoritative plan for evolving the `prism-doc-librarian` skill from a PRISM-specific tool into a **standalone, open-source-ready doc-governance project**. The generic librarian is NOT a skill folder inside PRISM's repo. It is a separate project that happens to have PRISM as its first consumer.
...
PRISM becomes a **consumer**: PRISM's repo contains only `skills/prism-doc-librarian/project_config.yaml` (the PRISM-specific config) and a thin wrapper pointer. PRISM's skill folder stops containing the actual librarian logic. PRISM's wrapper tells Claude: "load the doc-librarian skill from the standalone project and apply PRISM's config."
...
Phase A cannot start until the operator picks a destination for the standalone project. Three options exist; this plan does not pre-commit to one.

**Routing option 1 — additional Cowork mount.** The operator creates `/Users/chriskahnm5/projects/doc-librarian/` on the host Mac and mounts it as an additional Cowork folder (if supported) or switches the mount. Claude writes directly into the new mount from day one. Cleanest. Requires Cowork multi-mount or a mount swap.

**Routing option 2 — scaffold in Claude's temp workspace, hand off as archive.** Claude builds the standalone project in its working directory (`/sessions/vigilant-fervent-keller/doc-librarian/`). Not visible in the operator's Cowork file browser during the session. At end of Phase A, Claude produces a tarball or zip the operator can extract to wherever they want the real project to live. No mount changes required. Downside: the operator can't inspect files in real time through Cowork's native file UI.

**Routing option 3 — temporary extraction folder inside PRISM.** Claude scaffolds at `prism/_to-extract/doc-librarian/` with a README marking it for imminent removal. The operator cuts the folder out to a new standalone location on the host. Claude deletes the extraction folder from PRISM in the same commit that bootstraps PRISM's new thin-wrapper config. Maintains Cowork file visibility. Downside: transient git churn in PRISM's history (one commit adds the extraction folder, another removes it, both touching PRISM's tree for reasons unrelated to PRISM's goals).
...
**Goal:** bootstrap the standalone `doc-librarian` project. Extract generic logic from `skills/prism-doc-librarian/SKILL.md`. Make PRISM a consumer via a thin wrapper.
...
- `skills/prism-doc-librarian/project_config.yaml` — PRISM's concrete config
- `skills/prism-doc-librarian/SKILL.md` — rewritten as ~50 line wrapper that documents the dependency on the standalone doc-librarian project, loads the project_config, and points Claude at the generic skill
- `docs/librarian-buildout-plan-20260411-V1.0.md` remains in PRISM as the record of the decision (not the authoritative copy once Phase A completes — the authoritative copy lives in the standalone project)
...
- PRISM receives its rendered copy at `docs/diagrams/doc-librarian-dashboard-YYYYMMDD-V3.0.html`
...
**Goal:** repackage standalone `doc-librarian` as a Claude Code plugin, publish open-source (after PRISM's provisional patents are filed), add vector index, tie into the planned startup plugin bundle.
...
**Standalone project location.** Three routing options exist for Phase A. The operator must pick one at kickoff. This plan does not pre-commit to a choice.

**Cowork visibility during standalone work.** If the operator picks routing option 2 (temp workspace), the standalone project is invisible in Cowork's file browser during Phase A. The operator must rely on Claude's verbal reports plus the final handoff archive. Option 1 and option 3 preserve visibility.
...
1. The standalone `doc-librarian` project exists outside PRISM and has its own git history
...
|---|---|---|
| skills/prism-doc-librarian/SKILL.md | Phase A | Rewritten as thin wrapper pointing at standalone |
| skills/prism-doc-librarian/project_config.yaml (new) | Phase A | Created — PRISM's concrete config |
| docs/REGISTRY.yaml (PRISM) | Phase A, B, C, D | Schema compatibility; manifest entries |
| docs/diagrams/doc-librarian-dashboard-20260411-V2.1.html | Phase D | Superseded by V3.0 (rendered from standalone template) |
