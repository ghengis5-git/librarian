...
├── examples/                          # deferred — PRISM config serves as reference
│   ├── project_config.minimal.yaml    # Phase B
│   ├── project_config.full.yaml       # Phase B
│   └── project_config.prism.yaml      # Phase B
├── dashboard/                         # Phase D
├── site/                              # Phase E
└── docs/
    ├── architecture-*.md              # Phase B
    └── buildout-plan-*.md             # stays in PRISM as record of decision
```

PRISM is a **consumer**: PRISM's `skills/prism-librarian/SKILL.md` is a 110-line thin wrapper containing a condensed governance protocol and a CLI command table. Mechanical operations (audit, register, bump, status) delegate to the standalone Python CLI via `python -m librarian`. PRISM's REGISTRY.yaml `project_config` block remains the PRISM-specific configuration source.
...
Session 25 selected **Routing option 1 — additional Cowork mount** at `~/projects/librarian/` on the host Mac. Session 26 executed via a **hybrid workflow**: Cowork for governance/docs/registry, Claude Code terminal for Python/tests/git. Files were staged at `~/projects/prism/.librarian-port-staging/` and copied to the librarian repo by the operator.

Key discovery: the Cowork sandbox Python resolves to system 3.10.12, not the host's pyenv 3.13.12. All Python execution (tests, CLI, pip install) must happen in Claude Code terminal with the librarian venv activated.

Routing options 2 (temp workspace + archive handoff) and 3 (temporary extraction folder inside PRISM) were not needed.
...
| `examples/project_config.prism.yaml` | Deferred — PRISM's REGISTRY.yaml serves as reference | Low value until multi-project adoption |
...
| `docs/buildout-plan-*.md` — copy into standalone | Stays in PRISM only | Authoritative copy remains in PRISM; standalone is too young for its own governance docs |
...
| Not planned | `scripts/librarian-pre-commit-hook-20260411-V1.0.sh` | Ported from PRISM V1.3, adapted for standalone use |
| PRISM wrapper ~50 lines | PRISM wrapper 110 lines (CLI delegation) | Larger due to condensed governance protocol inline |
| `skills/prism-librarian/project_config.yaml` (new file) | Config stays in `docs/REGISTRY.yaml` `project_config` block | No separate config file needed — REGISTRY.yaml already has it |

**Commits:**
- `7d5fcab` (librarian) — Phase A inaugural: 20 files, 2,162 insertions
- `5389e83` (PRISM) — SKILL.md wrapper rewrite: 72 insertions, 412 deletions

**Test results:** 36/36 passing in 0.06s (librarian), 84/84 passing (PRISM).

**Regression check:** `python -m librarian --registry docs/REGISTRY.yaml audit` against PRISM's docs/ produces equivalent findings to the old inline skill. 39 files on disk, 40 registered, 26 legacy naming violations (expected — pre-convention files), 4 missing from operator/phase-specs/ (expected — not in tracked_dirs).
...
- `examples/` directory — minimal, full, and PRISM reference configs
...
- First generated manifest for PRISM
...
**Cowork sandbox limitations (discovered in Phase A).** The Cowork sandbox Python resolves to system 3.10.12, not the host's pyenv 3.13.12. All Python execution must happen in Claude Code terminal. The standalone librarian at `~/projects/librarian/` is not accessible from PRISM's Cowork session Read tool (different mount). This drove the CLI delegation wrapper design — the PRISM wrapper keeps a condensed governance protocol inline rather than pointing Claude at the generic skill file it cannot read from within PRISM's session.

**Cross-repo sync during iteration.** As Phase B adds a manifest generator, PRISM's registry remains the first test target. The standalone scripts need to read PRISM's `project_config` and write manifests into PRISM's repo. This cross-repo read/write must be explicit in every script's config (no hardcoded paths). Flagging this as a risk because it's a common drift source.
...
**PRISM tracked_dirs gap.** PRISM's `project_config.tracked_dirs` is currently `["docs/"]` only. Four files in `operator/phase-specs/` show as "missing" in audits because they are registered but live outside tracked dirs. Adding `"operator/"` and `"skills/"` to `tracked_dirs` is a deferred PRISM config fix, not a librarian code issue.
...
2. The generic skill has no PRISM content in normative sections — examples may reference PRISM but the skill works identically for any project supplying a valid `project_config` ✅
3. PRISM consumes the standalone skill via a thin wrapper — if the standalone project is missing, PRISM's wrapper fails in an obvious way rather than continuing to work by accident ✅ (CLI delegation: `python -m librarian` fails if package not installed)
...
|---|---|---|
| skills/prism-librarian/SKILL.md | Phase A ✅ | Rewritten as 110-line CLI delegation wrapper |
